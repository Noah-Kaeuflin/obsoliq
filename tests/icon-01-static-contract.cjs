...
const manifestPath = "assets/icons/obsoliq/icon-manifest.json";
const spritePath = "assets/icons/obsoliq/obsoliq-icon-sprite.svg";
const runtimePath = "js/ui/obsoliq-icon-system.js";
...
check(manifest.length === 39, `Expected 39 manifest records, received ${manifest.length}`);
check(new Set(manifestIds).size === 39, "Manifest IDs must be unique");
check(symbols.length === 39, `Expected 39 sprite symbols, received ${symbols.length}`);
check(new Set(spriteIds).size === 39, "Sprite symbol IDs must be unique");
...
check(fs.existsSync(path.join(root, "assets/icons/obsoliq/LICENSE-LUCIDE.txt")), "Lucide license must exist");
check(fs.existsSync(path.join(root, "assets/icons/obsoliq/README.md")), "Icon-pack README must exist");
