(function () {
  ...
  test("EX-UX-01.3 DOM follows the narrative order, avoids duplication and refreshes on Case switch", async assert => {
    ...
    assert.ok(children.indexOf(narrative) < children.indexOf(value), "The visual sequence should start with the existing narrative and Gross-to-Net value logic");
    assert.ok(children.indexOf(value) < children.indexOf(history), "Canonical Historical evidence should follow Gross-to-Net value logic");
    assert.ok(children.indexOf(history) < children.indexOf(decisionBasis), "Prioritization drivers should follow History");
    assert.ok(children.indexOf(decisionBasis) < children.indexOf(actionOptions), "Action options should follow the three primary evidence visuals");
    assert.ok(children.indexOf(actionOptions) < children.indexOf(work), "Work context should remain secondary to the existing action options");
