(function () {
  ...
  test("DF-UX-02.1 prioritized summary keeps sourceStates authoritative", async assert => {
    ...
    assert.equal(Object.keys(model.sourceStates).length, 3, "Full source states should remain available in the presentation model");
  ...
  test("DF-UX-02.1 open surface uses compact rows and accessible X close", async assert => {
    ...
    assert.equal(details.querySelectorAll(".data-foundation-source.actionable-missing").length, 2, "Missing extensions should render as actionable rows");
